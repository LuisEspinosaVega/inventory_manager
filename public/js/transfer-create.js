...
$("#item_search").on('change', function (e) {
    ...
    if (item_search != "") {
        ...
        $("#itemsTransferForm").trigger('reset');
    }
});
...
$("#btnAddItem").on('click', function (e) {
    ...
    if (item_id == "" || transfer_amount == "" || parseInt(transfer_amount) <= 0) {
        ...
        if (parseInt(transfer_amount) > parseInt(stock)) {
            $("#stockWarning").modal('show');
        }

        item.item_id = item_id;
        item.transfer_amount = transfer_amount;
        item.name = name;
...
$("#btnConfirmTransfer").on('click', function () {
    ...
    if (office_id == "" || mandated == "" || applicant == "" || destiny == "" ) {
        ...
        if(office_id == destiny){
            // verificar que el origen no sea el mismo que el destino
            $("#originDestinyAlert").removeClass('d-none');
                setTimeout(() => {
                    $("#originDestinyAlert").addClass('d-none');
                }, 1500);
        }else{
            ...
            if (itemsList.length == 0) {
                ...
                fetch('/inventory/transfers', {
                    ...
                    body: JSON.stringify({ office_id: office_id, destiny: destiny, mandated: mandated, applicant:applicant, comment:comment, items: itemsList })
                })
                    .then(() => {
                        window.location.href = "/inventory/transfers";
                    })
                    .catch((error) => {
                        console.log(error);
                    })
